...
def one_dim_trapezoidal(q: int, a=0, b=1, example_2=False):
    """
    ...
    Args:
        ...
        example_2 (bool, optional): Boolean only defined for the example with the periodic, derivable functions.
    ...
    if example_2:
        points = 0 + (b - a) * np.array(range(1, 2 ** q)) / (2 ** q)
...
def one_dim_newton_cotes(q: int, a=0, b=1):
    ...
    if q > 5:
        raise Exception("Please choose q < 6 for the Newton-Cotes approximation to avoid numerical errors.")
    ...
    weights, __ = newton_cotes(len(points) - 1, 1)
...
def monte_carlo_quad(q: int, a=0, b=1):
    ...
    # Changed to make calculation of prob. quad. faster.
    if q == 1:
        m = 1
    else:
        m = 2 ** (q - 1) + 1  # number of points where to evaluate function
...
def smolyak(f, weights: list, points: list,  q: int, dim: int, is_nested=False, example_2=False) -> list:
    """
    ...
    if len(weights) != (q - dim + 1) or len(points) != (q - dim + 1):
        raise Exception("The list of points and weights given for the Smolyak have incorrect lengths.")
    ...
    for i in range(number_approx):
        ...
        if isinstance(points[0], np.ndarray) and len(points[0].shape) == 1:
            ...
            if meshgrid_points[0].shape != meshgrid_weights[0].shape:
                ...
                raise Exception("The dimensions of the 1D gridpoints and weights do not match for the "
                                "vector i = " + str(i))
            ...
            if is_nested and i > 0 and dim < 5:
                ...
                positions_needed_evals, positions_old_evals, position_weights_of_saved_eval = \
                    slice_find(current_shape, old_shape, open_quad=example_2)
                ...
                if sum_tuple_shape(used_old_evals.shape, new_evals.shape) != function_values.shape:
                    raise Exception("Something went wrong with the usage of the nested information."
                                    ...
                                    "dim. points used: " + str(used_old_evals.shape) + "\n" +
                                    "dim. points new: " + str(new_evals.shape))
...
def sum_tuple_shape(old_shape: tuple, new_shape: tuple):
    """
    Sum up tuples
    ...
    if len(new_shape) != dim:
        raise Exception("The input data needs to have the same lengths.")
    ...
    for k_1 in range(dim):
        ...
        if old_shape[k_1] == new_shape[k_1]:
            ...
            # is not used because problems  with one dimensional vector
...
def sort_tuples(list_of_tuples: list, q: int):
    """
    Sort tuples

    We need this function to use the advantages of nested information, for this it is easier to first need to sort the
    list of tuples in a way, that we first evaluate the function in a systematical way. This sorting is done here.
...
def find_next_tuples_index(current_tuple: tuple, tuple_list: list):
    """
...
def slice_find(current_shape: tuple, nearest_shape: tuple, open_quad=False):
    """
    ...
    for tuple_dim in range(len(current_shape)):
        ...
        if current_shape[tuple_dim] == 3 and nearest_shape[tuple_dim] == 1:
            ...
            if current_shape[tuple_dim] > nearest_shape[tuple_dim] + 1 and nearest_shape[tuple_dim] != 1:
                ...
                position_weights_of_saved_evals.append(slice(1, current_shape[tuple_dim], 2))
...
# The following three methods were written, because the former way to find the possible combinations of one
# dimensional approximations used for the Smolyak-alg. (using permutations) appeared to be slow. Perhaps a
# little bit to much modularisation.
...
def next_step(current_vector: np.array, pointer_current_position: int, max_value: int):
    ...
    # Idf value in current pos. still is small enough, one can be added
...
def find_right_position(current_vector: np.array, pointer_current_position: int, max_value: int):
    """
    Faster combinatorics
...
def error_smolyak(f, quadrature: str, variables, q: int, function_string_error: str, a=0, b=1, give_parameters=False):
    ...
    if quadrature == "Trapezoidal" or "Newton-Cotes":
        ...
        for i in range(len(diff_options)):
            if not (diff_options[i] in diff_options[:i]):
                ...
                # It turned out, that this takes a long time, if there are to many points
                if dim < 4:
                    random_start_points = a + (b - a) * np.random.rand(3 * dim, dim)
                else:
                    # This  is set to a too low value. Reset to higher value if possible.
                    random_start_points = a + (b - a) * np.random.rand(1, dim)
    ...
    # Here the one dimensional error for the Monte-Carlo quadrature is estimated.
    # For this we take a (sufficiently?) big sample to estimate the variance of the approximation
    #    if quadrature == "Monte Carlo (nested)" or quadrature == "Monte Carlo (non-nested)":
    if quadrature == "None" or quadrature == "None":
        ...
        for i in range(dim):
            minimum.append(a)
            maximum.append(b)
        points = np.array(np.random.uniform(low=minimum, high=maximum, size=(32 * dim, dim)))
        sample = [f(*points[k_1]) for k_1 in range(len(points))]
        sigma = np.sqrt(np.var(sample))

        # Here we choose the 95% error-level/ 2 standard deviations
        B = 1
        C = 2 * sigma
        D = 1 / np.sqrt(2)
        E = C * ((D + 1) / D)
    ...
    if quadrature == "Quasi-Monte Carlo" or quadrature == "Monte Carlo (nested)" \
        ...
        if dim < 4:
            points = a + (b - a) * np.array(Lattice.gen_samples(64 * dim))
        else:
            points = a + (b - a) * np.array(Lattice.gen_samples(4 * dim))
        sample = [f(*points[k_1]) for k_1 in range(len(points))]
        sigma = np.sqrt(np.var(sample))
...
def fast_eps_cost(dim: int, B: float, C: float, D: float, F_0: float, F: float, epsilon: float, q=0, is_nested=True):
    """
    ...
    # Here some variables are commented out, that are defined in Tractability of Multivariate Problem_vol_2 Theorem 15.6
    # for the calculation of the estimation of the cost. If this output is wanted, the hashtags just could be removed.
    if is_nested:
        alpha_3 = alpha / 2 + 1
        alpha_2 = ((F_0 * (F - 1)) / F) ** (2 / (alpha + 2)) * ((np.e ** 2) / ((np.e - 1) * np.log(1 / D))) * \
                  (C / D) ** ((2 * alpha) / (2 + alpha))
        alpha_1 = alpha_2 * np.log(((np.e * C ** 2) / (2 * D ** 2 * np.log(1 / D)))) / 2
        alpha_0 = (dim / (2 * np.pi * (dim - 1))) ** ((alpha + 2) / 4) * ((F_0 * F) / (D ** 2))

    else:
        alpha_3 = alpha + 1
        alpha_2 = ((np.e ** 2 * F_0 ** (1 / (alpha + 1)) * H ** (alpha / (alpha + 1))) / ((np.e - 1) * np.log(1 / D)))
        alpha_1 = alpha_2 * np.log(((np.e * H) / (np.log(1 / D))))
        alpha_0 = (dim / (2 * np.pi * (dim - 1))) ** ((alpha + 1) / 2) * ((F_0 * F ** 2) / ((F - 1) * D))

    cost_estim = alpha_0 * (
                alpha_1 + alpha_2 * (np.log(np.sqrt(dim / (2 * np.pi * (dim - 1)))) + np.log(C / epsilon)) / (
                    dim - 1)) ** (alpha_3 * (dim - 1)) * (C / epsilon) ** alpha
    if q == 0:
        error = 0
    else:
        error = C * (B ** (dim - 1)) * (D ** (q - dim + 1)) * np.sum([(((E * D) / B) ** j) *
    ...
    return cost_estim, alpha, alpha_0, alpha_1, alpha_2, alpha_3, error


def epsilon_cost(function_string: str, variable_string: str, quadrature: str, epsilon: numbers, nested=False,
                 open_quad=False):
    """
    Epsilon cost
    ...
    F_0, F = cost_smolyak(0, dim, nested=nested, example_2=open_quad)
    ...
    q_epsilon = math.ceil(t_k * (dim - 1) / (np.log(1 / D)))
    cost_estim = alpha_0 * (
                alpha_1 + alpha_2 * (np.log(np.sqrt(dim / (2 * np.pi * (dim - 1)))) + np.log(C / epsilon)) / (
                    dim - 1)) ** (alpha_3 * (dim - 1)) * (C / epsilon) ** alpha
    return q_epsilon, cost_estim
...
def cost_smolyak(q: int, dim: int, nested=True, is_rand=False, example_2=False, give_param=False):
    """
    Cost calculation
    ...
    Args:
        ...
        example_2 (boolean, optional): Mainly defined for example 2. Decides, if the quadrature is open, default is
            False.
    ...
    if nested:
        m_q_d = F_0 ** dim * (F - 1) ** dim * sum(
            [F ** j * scipy.special.binom(j + dim - 1, dim - 1) for j in range(q - dim + 1)])
    else:
...
def controller_smolyak(function_string_control: str, variables_string_control: str, quadrature: str, q: int,
                       no_error=False, function_given=False, example_2=False, a=0, b=1):
    """
    Controller of project
    ...
    Args:
        ...
        function_given (boolean, optional): Needs to be set to true, if a callable function already is given.
            In this case the input function_string_control needs to be a callable function and variable_string_control
    ...
    # First the string given as function and tuple of variables is possessed to
    # a easily callable function

    if not function_given:
        f, variables, function_string_control = rewrite_function(function_string_control, variables_string_control)

    else:
        f = function_string_control
        variables = variables_string_control
    ...
    if quadrature == "Newton-Cotes":
        ...
        for i in range(1, (q - dim + 2)):
            points, weights = one_dim_trapezoidal(i, a, b, example_2=example_2)
            list_of_points.append(points)
            list_of_weights.append(weights)
        Smolyak_result = smolyak(f, list_of_weights, list_of_points, q, dim, example_2=example_2, is_nested=True)
        ...
        for i in range(1, (q - dim + 2)):
            ...
            if quadrature == "Monte Carlo (nested)" and i > 1:
                if i != 2:
                    ...
                    points_new, weights = monte_carlo_quad(2, a, b)

                    # Here a we produce the same structure like in the deterministic quadratures. With respect to the
                    # quadrature before every second point is new.
                    points = np.zeros(3)
                    points[1] = list_of_points[-1]

                    points[0:3:2] = points_new[:-1]
    ...
    if quadrature == "Quasi-Monte Carlo" or quadrature == "Monte Carlo (non-nested)":
        cost = cost_smolyak(q, dim, nested=False)
    else:
        cost = cost_smolyak(q, dim, nested=True)
    if example_2:
        cost = cost_smolyak(q, dim, nested=True, example_2=example_2)
